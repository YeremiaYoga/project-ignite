// app/characters-maker/view/[privateId]/CharacterDetailClient.jsx
"use client";
...
function mapDbCharacterToViewModel(db, idFromRoute) {
  if (!db) {
    return {
      ...
      personality_data: [],
      meta_data: [],
      private_id: idFromRoute,
      public_id: "",
    };
  }

  console.log("db character:", db);
...
export default function CharacterDetailClient({ privateId }) {
  ...
  useEffect(() => {
    if (!privateId) {
      setDenyReason("Invalid character id.");
      setLoading(false);
      return;
    }
    ...
    const load = async () => {
      try {
        ...
        if (!meRes.ok) {
          setDenyReason("You must be signed in to view this character.");
          setLoading(false);
          return;
        }
        ...
        // 2) fetch character private
        const charRes = await fetch(
          `${API_BASE}/characters/private/${privateId}`,
          {
            method: "GET",
            credentials: "include",
          }
        ...
        if (charRes.status === 404) {
          setDenyReason("Character not found.");
          setLoading(false);
          return;
        }

        if (!charRes.ok) {
          setDenyReason("Failed to load character.");
          setLoading(false);
          return;
        }
        ...
        // 3) cek owner — SESUAIKAN dengan schema kamu
        // kalau kamu pakai "user_id" di table characters:
        const ownerId = dbChar.user_id;
        ...
        // 4) success
        const mapped = mapDbCharacterToViewModel(dbChar, privateId);
        setCharacter(mapped);
        setLoading(false);
      } catch (err) {
  ...
  if (denyReason && !character) {
    return <div className="p-6 text-gray-200 text-sm">{denyReason}</div>;
  }
