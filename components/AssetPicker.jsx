...
export default function AssetPicker({ isOpen, initialPath = "", onClose, onSelect }) {
  const [loading, setLoading] = useState(false);
  const [cwd, setCwd] = useState(initialPath);
  const [data, setData] = useState({ path: "", folders: [], files: [], breadcrumbs: [] });

  const load = async (p) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/assets/list?path=${encodeURIComponent(p)}`, { cache: "no-store" });
    ...
    <div className="fixed inset-0 z-50">
      ...
      <div
        className="
          ...
          max-h-[85vh] flex flex-col  /* <- batas tinggi & layout kolom */
        "
      >
        ...
        <div className="flex items-center justify-between border-b border-slate-800 px-4 py-3">
          <div className="flex items-center gap-2 text-sm">
            ...
            <button className="hover:underline" onClick={() => load("")}>assets</button>
            {data.breadcrumbs?.slice(1).map((bc) => (
              <React.Fragment key={bc.path}>
                <ChevronRight className="w-4 h-4 opacity-50" />
                <button className="hover:underline" onClick={() => load(bc.path)}>{bc.name}</button>
              </React.Fragment>
            ))}
          </div>
          <button onClick={onClose} className="rounded-md p-1 hover:bg-slate-800">
            <X className="w-5 h-5" />
          </button>
        </div>
          ...
          {loading ? (
            ...
            <>
              ...
              {data.folders?.length > 0 && (
                <>
                  <h4 className="mb-2 text-xs uppercase tracking-wider text-slate-400">Folders</h4>
              ...
              {/* Images */}
              <h4 className="mb-2 text-xs uppercase tracking-wider text-slate-400">Images</h4>
              {data.files?.length === 0 ? (
                <div className="py-10 text-center text-slate-500 text-sm">No images here.</div>
              ) : (
                <div className="grid grid-cols-3 gap-3 sm:grid-cols-4 md:grid-cols-6">
                  {data.files.map((file) => (
                    <button
                      key={file.path}
                      onClick={() => { onSelect(`/assets/${file.path}`); onClose(); }}
                      title={file.path}
